...
- (void)queryMultiBasicBusinessInfoForTerm:(NSString *)term inZipCode:(NSString *)zipCode completionHandler:(void (^)(NSArray *businessJSON, NSError *error))completionHandler {
    NSLog(@"Querying the Search API with term \'%@\' and zipcode \'%@'", term, zipCode);
    
    // Make a first request to get the serach results with the passed term and zipcode
    NSURLRequest *searchRequest = [self _searchRequestWithTerm:term location:zipCode];
    NSURLSession *session = [NSURLSession sharedSession];
    [[session dataTaskWithRequest:searchRequest completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
        NSHTTPURLResponse *httpResponse = (NSHTTPURLResponse *)response;
        ...
        if (!error && httpResponse.statusCode == 200) {
            ...
            if ([businessArray count] > 0) {
                // for all listed business in Array
...
- (void)queryMultiDetailBusinessInfoForTerm:(NSString *)term inZipCode:(NSString *)zipCode completionHandler:(void (^)(NSArray *businessJSON, NSError *error))completionHandler {
    NSLog(@"Querying the Search API with term \'%@\' and zipcode \'%@'", term, zipCode);
    ...
    [[session dataTaskWithRequest:searchRequest completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
        ...
        if (!error && httpResponse.statusCode == 200) {
            ...
            if ([businessArray count] > 0) {
                // for each business in Array search for it's details
                NSMutableArray *bizDetailArray = [[NSMutableArray alloc] init];
                // thread async
                dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH,0), ^{
                    // request for each async
                    dispatch_group_t requestGroup = dispatch_group_create();
                    for (id biz in businessArray) {
                        dispatch_group_enter(requestGroup);
                        NSString *bizID = biz[@"id"];
                        [self queryBusinessInfoForBusinessId:bizID completionHandler:^(NSDictionary *bizInfo, NSError *error) {
                            //biz[@"details"] = bizInfo;
                            [bizDetailArray addObject:bizInfo];
                            dispatch_group_leave(requestGroup);
                        }];
                    }
                    dispatch_group_wait(requestGroup, DISPATCH_TIME_FOREVER);
                    completionHandler(bizDetailArray, error);
                });
